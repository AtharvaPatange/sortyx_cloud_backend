        ...
        try:
            ...
            if api_key:
                ...
                try:
                    ...
                    logger.info("✅ Gemini model 'gemini-1.5-flash' is accessible")
                except Exception as test_error:
                    logger.warning(f"⚠️ Could not access gemini-1.5-flash, trying gemini-pro: {test_error}")
                    try:
                        test_model = genai.GenerativeModel('gemini-pro')
                        logger.info("✅ Gemini model 'gemini-pro' is accessible")
                    except Exception as fallback_error:
                        logger.error(f"❌ Could not access any Gemini model: {fallback_error}")
            else:
